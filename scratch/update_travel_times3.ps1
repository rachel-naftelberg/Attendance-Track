$excelFile = "C:\Users\um624\Downloads\traveltimes.xlsx"
Write-Host "Parsing Excel file: $excelFile"

# Create a temporary directory
$tempDir = Join-Path "C:\Users\um624\.gemini\antigravity\scratch\iec-attendance-app" "temp_excel5"
...
function Get-Minutes($val) {
    ...
    # Handle scientific notation like 2.0833E-2
    if ($val -like "*E-*") {
        $parts = $val -split "E-"
        $base = [double]$parts[0]
        $exp = [int]$parts[1]
...
foreach ($r in $records) {
    ...
    # G = dedicated arrival time, H = dedicated return time, F = general travel time
    $arrivalMin = Get-Minutes $r.G
    $returnMin  = Get-Minutes $r.H
    $totalTime  = Get-Minutes $r.F
    $distance   = $r.E
    if ($null -eq $distance) { $distance = "" }

    # Store raw values: [arrivalMin, returnMin, distance, generalTime]
    # arrivalMin/returnMin = 0 means no dedicated time in Excel -> JS will use generalTime + show asterisk
    if (-not $travelTimes.ContainsKey($city)) {
        $travelTimes[$city] = @{}
    }
    $travelTimes[$city][$dest] = @($arrivalMin, $returnMin, $distance, $totalTime)
}
